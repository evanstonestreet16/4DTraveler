...
test('Kyoto → Nijō → objects → transcript → overview', async ({ page }) => {
  ...
  const expectedOverview = world.scene.overviewImage!.desktop;
  await expect(overview).toHaveAttribute(
    'data-asset-url',
    expectedOverview.url,
  );
  await expect
    .poll(() =>
  ...
  await expect(overview).toHaveAttribute('data-asset-url', initialSource!);
});
...
test('failed overview and panorama images retain usable places, object stories, and retry', async ({
  ...
  const failedPaths = new Set(
    [overview.desktop, panorama.desktop].map(
      (asset) => new URL(asset.url, 'http://local').pathname,
    ),
  );
