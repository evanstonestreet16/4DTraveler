...
test('overview labels stay inside the viewport while stems retain the authored anchors', async ({
  page,
}) => {
  await page.goto('/');
  ...
  for (const poi of world.pois) {
    ...
    const anchor = overviewMarkerPosition(
      overview.markers[poi.id],
      overview.desktop,
      viewport,
    );
    if (
