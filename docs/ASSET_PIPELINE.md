...
## Content-versioned byte caching

Hero URLs carry `?v=` followed by the first twelve SHA-256 hex digits of the GLB. The loader uses the browser's HTTP cache (`force-cache`) for re-entry; it still creates and disposes a fresh parsed scene per visit, so it never shares mutable highlight materials between worlds. After regenerating the hero, update the scene URL's version from `npm run assets:budget`; `npm run assets:validate` rejects an out-of-date version. Failure tests match model requests including query strings. Keep the fixture URL unversioned for development-only validation.

The asset budget command reports raw, gzip and Brotli byte sizes. The palette has no texture payload, the generator merges repeated geometry by material, and the full GLB is below one third of the 8 MiB budget. This avoids adding a runtime Draco/Meshopt/KTX2 decoder for this slice. Production hosting can apply HTTP gzip/Brotli to GLB/JS responses and `Cache-Control: public, max-age=31536000, immutable` to content-versioned model URLs; retain revalidation for HTML. Verify the actual `Content-Encoding`/cache response in the profiling report rather than assuming those headers are enabled by Vite preview. Source kit/preview scripts live outside `public` and are excluded from the runtime bundle.

The hero also sets optional `compressedUrl` to the same versioned path with `.glb.gz`. `npm run assets:hero` produces the gzip file deterministically. It is transferred as gzip bytes and decoded with the native Compression Streams API when available; a plain GLB URL is always retained for unsupported browsers and failed compressed downloads. Servers may instead send Content-Encoding: gzip, in which case fetch already decodes it and the GLB header is recognized directly. No new historical or selection IDs are introduced by either URL.
