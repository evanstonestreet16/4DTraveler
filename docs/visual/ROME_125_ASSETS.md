...
Selection now stops at the nearest visible surface, including scenery without metadata, and updates hover while moving between meshes within one GLB. This prevents inspecting hidden objects through walls. Browser coverage checks the actual arcade raycast, all nine object paths, repeated three-POI transitions, low-detail recovery, and ambience play/pause/error/retry/exit. Cardinal and responsive screenshots are stored in `docs/evidence/rome-p2/`.
