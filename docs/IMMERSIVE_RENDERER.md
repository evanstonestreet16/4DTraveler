...
`tests/immersive.spec.ts` covers repeated CSS transitions with live audio and DOM identity checks; camera target/aspect via real marker projection; native browser exit and Escape; rejected and unavailable APIs; reserved composition bounds; graphics recovery; and navigation cleanup. Run after a production build with `npm run test:e2e -- tests/immersive.spec.ts`.

Chromium acceptance uses one desktop context. Safari browser chrome, operating-system fullscreen gestures, and moving a window between physical monitors remain manual checks for release verification.

Integrated with the actual hero GLB, atmosphere and DPR listener: all formatting/lint/typecheck/build checks, 14 unit tests and 13 production browser tests pass. [Portrait](evidence/immersive/immersive-390.png) and [landscape](evidence/immersive/immersive-844.png) captures show the same selected furnace and reserved canvas; both were visually inspected. The desktop test verifies the same canvas/audio DOM identity, continuing playback, unchanged selection and camera projection across two full enter/exit cycles, including native exit and rejected Fullscreen API requests.
