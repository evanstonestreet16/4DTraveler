...
## Compilation control and budget disposition

A separate [JIT-disabled diagnostic](jitless-diagnostic/README.md) repeated five warmup plus five measured visits with the same build, browser, Apple M3 GPU, quality, viewport and DPR. The sole launch change was `--js-flags=--jitless`, verified by zero InstructionStream nodes in its snapshots. Its unchanged heap check passed: cycles 2–5 varied by 40,752 bytes and ended 40,568 bytes higher, compared with the normal control's 529,108-byte rise. Retained Three counts, closures, backing storage and GPU resources stayed flat. The diagnostic is not a proposed runtime setting and does not replace normal-browser performance measurements.

This evidence supports evaluating the initial resource budget through application ownership: no retained world-graph growth, no unreleased GPU handles, and stable heap under a compilation control. Normal total-heap growth remains an explicit review finding; neither the profiler threshold nor its failed flags have been relaxed. Reliability can review the raw normal/control traces and this proposed distinction before approving the resource budget. Physical-phone performance remains a separate unverified gate.
