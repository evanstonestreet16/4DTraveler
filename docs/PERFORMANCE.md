...
For the native laptop GPU, request the browser default backend and verify the **observed** WebGL renderer and CDP GPU device in the generated JSON. A requested hardware backend may still fall back to software. On this Apple M3, the headless shell selected SwiftShader while headed Chromium reported the native ANGLE Metal renderer; always inspect the observed renderer.
...
- A desktop run on an observed hardware GPU compares active-frame p95 with 22.22 ms (the 45 FPS target). Software rendering is reference evidence only.
...
The profiler now measures the desktop viewport only; the mobile row is retained baseline history. The hero scene is more complex than Dummy v0. This baseline documents that change; it is not an equivalent-scene optimization comparison. Capture another **before** run after representative hero assets and atmosphere exist, before quality/chunking changes. Capture **after** with the same observed GPU, dimensions, DPR, quality and power conditions. Do not call an Apple-GPU result a speedup over SwiftShader. Use `PROFILE_BASELINE` to point the generated summary at the correct comparable directory; inspect provenance before interpreting differences.

The representative [hero-plus-atmosphere before evidence](evidence/performance/before/README.md) and [optimized measurements](evidence/performance/after/README.md) include Apple M3 Metal and explicit SwiftShader runs. Apple M3 High hero readiness changed from 1511 ms to 883 ms while active-frame p95 stayed at 17.6 ms. GPU handles returned to baseline on every leave, retained world-object counts stayed flat, and cached model re-entry transferred zero network bytes. Total post-GC heap still grew, including with `PROFILE_WARMUP_CYCLES=5`; the evidence preserves this failed check and the investigation into JIT code growth.
